...
def create_resume_txt(data):
    ...
    with open(filename, "w") as f:
        f.write(f"<b><font size=26>{name}</font></b>\n\n\n\n\n")
        f.write(f"Email: <a href='mailto:{email}'>{email}</a>\n")
        f.write(f"Phone: {phone}\n")
        f.write(f"{address}\n")
        f.write(f"<b><font size=18>Summary</font></b>\n\n")
        f.write(f"{summary}\n")
        f.write("Experience:\n\n")
        for exp in experience:
            f.write(f"<b><font size=14>{exp[0]}</font></b>\n\n")
            f.write(f"Duration: {exp[1]}\n")
            f.write(f"Work Summary: {exp[2]}\n")
        f.write("Education:\n\n")
        for edu in education:
            f.write(f"<b><font size=14>{edu[0]}</font></b>\n\n")
        ...
        for cert in certifications:
            f.write(f"<b><font size=14>{cert[0]}</font></b>\n\n")
            f.write(f"Platform: {cert[1]}\n")
            f.write(f"Subject: {cert[2]}\n")
